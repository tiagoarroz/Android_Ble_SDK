...
//            Logger.t(TAG).i("onDraw: " + stopX);
    ...
    private void drawPath(Canvas canvas, PointF[] mPoints, int pointX, int item_contents) {
        ...
        if (pointX <= 0) {
            leftLength = 0;
            middleLength = ecgLineSpace;
            rightLength = length - middleLength;
            ...
            for (int i = 0; i < mPoints.length; i++) {
                if (i < ecgLineSpace) {
                    ...
                    arrayRight[i - ecgLineSpace] = mPoints[i];
                }
            }
        } else if (pointX + ecgLineSpace >= length) {
            leftLength = pointX;
            middleLength = pointX + ecgLineSpace - length;
            rightLength = 0;
            ...
            leftLength = pointX;
            middleLength = ecgLineSpace;
            rightLength = length - pointX - middleLength;
            arrayLeft = new PointF[leftLength];
            arrayMiddle = new PointF[middleLength];
            arrayRight = new PointF[rightLength];
    ...
    public synchronized void changeData(int[] data, int item_content) {
        ...
        for (int i = linePositionX; i < linePositionX + item_contents; i++) {
            ...
            if (data[i - linePositionX] == Integer.MAX_VALUE) continue;
